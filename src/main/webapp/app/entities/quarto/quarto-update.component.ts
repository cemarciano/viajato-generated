...
export class QuartoUpdateComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.hotelService.query({ filter: 'quarto-is-null' }).subscribe(
            (res: HttpResponse<IHotel[]>) => {
                if (!this.quarto.hotel || !this.quarto.hotel.id) {
                    this.hotels = res.body;
                } else {
                    this.hotelService.find(this.quarto.hotel.id).subscribe(
                        (subRes: HttpResponse<IHotel>) => {
                            this.hotels = [subRes.body].concat(res.body);
                        },
