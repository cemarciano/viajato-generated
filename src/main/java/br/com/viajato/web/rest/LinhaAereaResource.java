...
public class LinhaAereaResource {
    ...
    @Timed
    public List<LinhaAerea> getAllLinhaAereas(@RequestParam(required = false) String filter) {
        if ("voo-is-null".equals(filter)) {
            log.debug("REST request to get all LinhaAereas where voo is null");
            return StreamSupport
                .stream(linhaAereaRepository.findAll().spliterator(), false)
                .filter(linhaAerea -> linhaAerea.getVoo() == null)
