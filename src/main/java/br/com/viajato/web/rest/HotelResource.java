...
public class HotelResource {
    ...
    @GetMapping("/hotels")
    @Timed
    public List<Hotel> getAllHotels(@RequestParam(required = false) String filter) {
        if ("quarto-is-null".equals(filter)) {
            log.debug("REST request to get all Hotels where quarto is null");
            return StreamSupport
                .stream(hotelRepository.findAll().spliterator(), false)
                .filter(hotel -> hotel.getQuarto() == null)
