...
public class LocadoraResource {
    ...
    @Timed
    public List<Locadora> getAllLocadoras(@RequestParam(required = false) String filter) {
        if ("carro-is-null".equals(filter)) {
            log.debug("REST request to get all Locadoras where carro is null");
            return StreamSupport
                .stream(locadoraRepository.findAll().spliterator(), false)
                .filter(locadora -> locadora.getCarro() == null)
